...
import { GET_PROJECTS, GET_PROJECT_WITH_SAMPLING_POINT } from '../../lib/queries';
  ...
  const handleUploadData = async () => {
    try {
      ...
      const { data: projectData } = await client.query({
        query: GET_PROJECT_WITH_SAMPLING_POINT,
        variables: { id: samplingPoint },
      });

      // Check if projectData is available
      if (projectData) {
        // Access the necessary fields from projectData
      /*   return {
          identifier: projectData.samplingPoint.identifier,
          projectName: projectData.samplingPoint.project

        } */
        const identifier = projectData.samplingPoint.identifier;
        const project = projectData.samplingPoint.project;
